    ...
    <div className="space-y-4">
        ...
        {[
          ...
          return (
            <motion.div
              ...
              viewport={{ once: true }}
              className={`p-3 rounded-xl glass border ${item.bg} flex items-center justify-between transition-all duration-300 group`}
            >
              <div className="space-y-0.5">
                <span className="text-[10px] text-slate-400 font-mono uppercase tracking-wider block">
                  {item.label}
                </span>
                <span className="text-xl md:text-2xl font-heading font-bold text-slate-100 block group-hover:scale-105 transition-transform duration-200 origin-left">
                  {item.value}
                </span>
              </div>
              <div className={`p-2 bg-slate-900 rounded-lg border border-slate-800 ${item.color}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="lg:col-span-5 p-4 rounded-xl glass border border-slate-800 flex flex-col justify-between"
        >
          <div>
            <h3 className="text-xs font-heading font-semibold uppercase tracking-wider text-slate-400 mb-3 flex items-center gap-2">
              <FolderCode className="w-3.5 h-3.5 text-emerald-500" />
              Languages Breakdown
            </h3>

            {/* Visual breakdown bar */}
            <div className="w-full h-2 bg-slate-900 rounded-full overflow-hidden flex mb-4">
          ...
          <div className="border-t border-slate-800/80 pt-3 mt-4 text-[10px] text-slate-500 font-mono text-center">
        ...
        <motion.div
          ...
          transition={{ duration: 0.4 }}
          viewport={{ once: true }}
          className="lg:col-span-7 p-4 rounded-xl glass border border-slate-800 flex flex-col justify-between"
        >
          <div>
            <h3 className="text-xs font-heading font-semibold uppercase tracking-wider text-slate-400 mb-3 flex items-center gap-2">
            ...
            <div className="overflow-x-auto pb-2">
              ...
              <div
                className="grid grid-flow-col gap-[3px] w-max mx-auto"
                style={{ gridTemplateRows: "repeat(7, minmax(0, 1fr))" }}
              >
                {contributionCells.map((cell, idx) => {
                  const tooltipText = cell.date 
                    ? `${cell.count} contributions on ${new Date(cell.date).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}` 
                    : "No contributions";
                  return (
                  <div
                    key={idx}
                    className={`w-2.5 h-2.5 rounded-[2px] border ${getIntensityColor(
                      cell.level
            ...
            <div className="flex justify-between items-center mt-3 text-[10px] font-mono text-slate-500">
              ...
              <div className="flex items-center gap-1 select-none">
                <span>Less</span>
                <span className="w-2.5 h-2.5 bg-slate-950 border border-slate-900/60 rounded-[2px]" />
                <span className="w-2.5 h-2.5 bg-emerald-900/50 border border-emerald-950/20 rounded-[2px]" />
                <span className="w-2.5 h-2.5 bg-emerald-700/70 border border-emerald-800/40 rounded-[2px]" />
                <span className="w-2.5 h-2.5 bg-emerald-500 border border-emerald-600/55 rounded-[2px]" />
                <span className="w-2.5 h-2.5 bg-emerald-400 border border-emerald-500 rounded-[2px]" />
                <span>More</span>
              </div>
            </div>
          </div>

          <div className="border-t border-slate-800/80 pt-3 mt-4 flex justify-between items-center text-[10px] font-mono text-slate-500">
